...
import { AnswerData, AnswerDataDocument, AnswerRepository, UpdateAnswerData } from "../types/event-types";
...
export class MongoDBAnswerRepository implements AnswerRepository {
    ...
    async updateEvent(eventData: UpdateAnswerData): Promise<boolean> {
        ...
        const updated = await this.answerModel.updateOne({ _id }, { $set: updateFields });
